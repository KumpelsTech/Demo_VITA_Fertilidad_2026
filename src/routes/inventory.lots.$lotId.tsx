...
import {
  ArrowLeft, Package, Snowflake, MapPin, Thermometer, AlertTriangle,
  ShieldAlert, ScanLine, Activity, Calendar, User, Sparkles, ChevronRight,
} from "lucide-react";
...
export const Route = createFileRoute("/inventory/lots/$lotId")({
  ...
  notFoundComponent: () => (
    <div className="p-10 text-center">
      ...
      <Link to="/inventory" className="text-primary text-sm font-medium hover:underline mt-2 inline-block">
        Back to inventory
      </Link>
    </div>
  ),
...
function LotDetailPage() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-6">
      {/* Header */}
      <Link to="/inventory" className="inline-flex items-center gap-1.5 text-[11px] text-muted-foreground hover:text-foreground mb-4">
      ...
      <div className="flex items-start justify-between gap-6 flex-wrap mb-6">
        <div className="flex items-start gap-4">
          <div className={cn(
            ...
            lot.status === "expired" || lot.status === "blocked" ? "bg-critical/10" :
            lot.status === "quarantined" ? "bg-warning/10" : "bg-accent"
          )}>
            <Package className={cn(
              "size-6",
              lot.status === "expired" || lot.status === "blocked" ? "text-critical" :
              lot.status === "quarantined" ? "text-warning" : "text-primary"
            )} />
          </div>
          <div>
      ...
      {lot.invimaAlert && (
        <div className="rounded-xl border border-critical/30 bg-critical/5 p-4 mb-6 flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-critical uppercase tracking-wider">Regulatory alert</p>
            <p className="text-sm text-foreground mt-1">{lot.invimaAlert}</p>
            <p className="text-[11px] text-muted-foreground mt-1">
              Lot was automatically blocked. 0 patients impacted to date. Quality team review in progress.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4">Stock allocation</h3>
            ...
            <div className="h-2 bg-muted rounded-full overflow-hidden flex">
              <div className="bg-success h-full" style={{ width: `${(lot.available / lot.total) * 100}%` }} />
              <div className="bg-primary h-full" style={{ width: `${(lot.reserved / lot.total) * 100}%` }} />
              <div className="bg-muted-foreground/50 h-full" style={{ width: `${(lot.consumed / lot.total) * 100}%` }} />
            </div>
            <div className="flex items-center gap-4 mt-3 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-success" /> Available</span>
              <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-primary" /> Reserved</span>
              <span className="flex items-center gap-1"><span className="size-2 rounded-sm bg-muted-foreground/50" /> Consumed</span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-gradient-to-br from-accent/40 to-card p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-primary mb-1">Kumpels AI</p>
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <ol className="relative px-5 py-5 space-y-5">
              {movements.length === 0 && (
                <p className="text-xs text-muted-foreground">No movements recorded for this lot yet.</p>
              )}
              {movements.map((mv, i) => {
                ...
                return (
                  <li key={mv.id} className="relative pl-6">
                    ...
                    <p className="text-xs text-foreground mt-1">
                      <span className="font-semibold">{mv.quantity} {mv.unit}</span> · {mv.from} → {mv.to}
                    </p>
                    <p className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-2 flex-wrap">
                      <span className="inline-flex items-center gap-1"><User className="size-3" /> {mv.user}</span>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4">Lot identity</h3>
            <dl className="space-y-3 text-xs">
              ...
              <Row label="Expiry" value={lot.expiry} mono accent={lot.daysToExpiry < 60 ? "warning" : undefined} />
              <Row label="Unit cost" value={`$${lot.cost}`} mono />
            </dl>
          </div>

          <div className="rounded-xl border border-border bg-card p-5">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4">Location</h3>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-1.5">
              {["Reserve for procedure", "Transfer to location", "Open quality review", "Request destruction"].map((a) => (
                <button key={a} className="w-full flex items-center justify-between px-3 h-9 text-xs font-medium rounded-md hover:bg-secondary text-left transition-colors">
                  {a}
...
function Stat({ label, value, unit, tone }: { label: string; value: number; unit: string; tone: "neutral" | "primary" | "success" }) {
  return (
    <div>
...
function Row({ label, value, mono, accent }: { label: string; value: string; mono?: boolean; accent?: "warning" }) {
  return (
