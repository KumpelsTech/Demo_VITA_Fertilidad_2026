    ...
    <div className="p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-1">Pipeline of all active cycles · milestone-driven</p>
        </div>
        <div className="flex items-center gap-1 text-[11px]">
          {["All", "At risk", "This week", "Long Agonist", "Antagonist", "FET"].map((f, i) => (
            <button key={f} className={cn(
              "px-2.5 h-7 rounded-md transition-colors",
              i === 0 ? "bg-accent text-primary font-medium" : "text-muted-foreground hover:bg-secondary",
            )}>
              {f}
            </button>
          ))}
        ...
        {journeys.map((j) => (
          ...
          >
            <div className="flex items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-3 min-w-0">
                <div className="size-10 rounded-full bg-accent text-primary flex items-center justify-center text-[12px] font-semibold shrink-0">
                  {j.patient.split(" ").map((n) => n[0]).slice(0, 2).join("")}
                </div>
                <div className="min-w-0">
                  <h3 className="text-[14px] font-semibold tracking-tight truncate">{j.patient}</h3>
                  <p className="text-[11px] text-muted-foreground truncate">{j.id} · {j.protocol}</p>
                </div>
              </div>
            ...
            <div className="relative">
              ...
              <div className="absolute left-0 top-[10px] h-[3px] bg-primary rounded-full" style={{ width: `${(j.progress / STAGES.length) * 100}%` }} />
...
function RiskBadge({ risk }: { risk: "low" | "medium" | "high" }) {
  if (risk === "low") {
    return <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-success/10 text-success">On track</span>;
  }
  if (risk === "medium") {
    return <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-accent text-primary">Monitor</span>;
  }
  return (
