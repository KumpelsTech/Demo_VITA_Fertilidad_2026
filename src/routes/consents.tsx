...
function ConsentsPage() {
  ...
  const handleSend = async (key: string) => {
    ...
    setDocuments((docs) =>
      docs.map((d) =>
        d.doc + d.id === key
          ? { ...d, status: "pending", date: new Date().toISOString().slice(0, 10) }
          : d,
      ),
    );
  };
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-1">
            Digital signatures, multilingual templates, WhatsApp delivery
          </p>
        </div>
        <button
          onClick={() =>
            notify("New document composer coming soon", "Use a template or create from scratch")
          }
      ...
      <div className="rounded-xl border border-accent bg-accent/40 p-3.5 flex items-start gap-2.5">
        ...
        <p className="text-[12px] text-foreground/80">
          ...
          Sofia Park has an FET scheduled tomorrow without an embryo storage consent. Auto-reminder
          queued for 18:00 (highest response window).
        </p>
      </div>
            ...
            <tbody>
              {documents.map((doc) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-4 text-muted-foreground font-mono tabular-nums">
                    {doc.date}
                  </td>
                  <td className="py-2.5 px-4 text-right">
                    {doc.status !== "signed" && (
                      ...
                      >
                        ...
                        {doc.status === "pending" ? "Resend" : "Send"}
                      </button>
                    )}
                  </td>
                </tr>
